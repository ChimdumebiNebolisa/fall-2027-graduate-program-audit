...
from graduate_audit.professor_mapping import FACULTY_ROSTERS, PROGRAM_FACULTY_ROSTERS  # noqa: E402
...
REENTRY_RAW_PATHS = (
    ...
    REPO_ROOT / "data/raw/pass2/stage_04_reentry_03.json",
    REPO_ROOT / "data/raw/pass2/stage_04_reentry_04.json",
    REPO_ROOT / "data/raw/pass2/stage_04_reentry_05.json",
    ...
    REPO_ROOT / "data/raw/pass2/stage_04_reentry_10.json",
    REPO_ROOT / "data/raw/pass2/stage_04_reentry_11.json",
    REPO_ROOT / "data/raw/pass2/stage_04_reentry_12.json",
    REPO_ROOT / "data/raw/pass2/stage_04_reentry_13.json",
...
def build() -> dict[str, object]:
    ...
    for program in sorted(programs, key=lambda row: (row["region"], row["institution_name"], row["candidate_program_id"])):
        ...
        roster = PROGRAM_FACULTY_ROSTERS.get(
            program_id_value, FACULTY_ROSTERS[program["institution_name"]]
        )
        ...
        for name, themes, is_primary in candidate_specs:
            ...
            row = {
                ...
                "unresolved_question": (
                    "Confirm that the current general recruiting statement still applies to Fall 2027 and that funded supervision capacity is available."
                    if qualifies and strongest["recruiting_status"] == "Confirmed recruiting" else
...
def write_report(result: dict[str, object]) -> None:
    ...
    lines = [
        ...
        "",
        (
            f"- Stage 4 re-entry 15 evaluated {counts['latest_reentry_programs']} newly eligible routes and retained "
            f"{counts['latest_reentry_retained_match_rows']} fully verified strong lead matches; "
            f"{counts['latest_reentry_zero_match_programs']} new routes remain without a retained match. Across all fifteen "
...
def main() -> int:
    ...
    stage_status["4_reentry_03"] = "complete" if result["status"] == "PASS" else "failed"
    stage_status["4_reentry_04"] = "complete" if result["status"] == "PASS" else "failed"
    stage_status["4_reentry_05"] = "complete" if result["status"] == "PASS" else "failed"
    ...
    stage_status["4_reentry_10"] = "complete" if result["status"] == "PASS" else "failed"
    stage_status["4_reentry_11"] = "complete" if result["status"] == "PASS" else "failed"
    stage_status["4_reentry_12"] = "complete" if result["status"] == "PASS" else "failed"
    stage_status["4_reentry_13"] = "complete" if result["status"] == "PASS" else "failed"
    ...
    pass2.update({
        ...
        "stage_04_reentry_completed": 15 if result["status"] == "PASS" else 14,
    ...
    update_progress(
        progress_path,
        current_phase="pass2_stage_04_reentry_15_complete" if result["status"] == "PASS" else "pass2_stage_04_reentry_15_failed",
        pass2=pass2,
    )
    ...
    unresolved = [
        ...
        "Stage 5 scoring still covers the prior 136-program roster; all seven new routes remain intentionally absent until Stage 5 re-entry 15 is executed.",
    ]
    ...
    manifest = {
        ...
        "schema_version": PASS2_SCHEMA_VERSION,
        "stage": 4,
        "run_type": "faculty_reentry_15",
        "name": "Deep professor and department fit mapping — re-entry 15",
        ...
        "validation": {
            ...
            "full_suite_boundary": {
                ...
                "result": "135 passed, 1 expected downstream Stage 5 coverage failure",
                "unresolved_stages": [5],
            },
        },
        "counts": result["counts"],
