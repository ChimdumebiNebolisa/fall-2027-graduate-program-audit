...
def write_report(result: dict[str, object]) -> None:
    ...
    lines = [
        ...
        "",
        (
            f"- Stage 4 re-entry 12 evaluated {counts['latest_reentry_programs']} newly eligible routes and retained "
            f"{counts['latest_reentry_retained_match_rows']} fully verified strong lead matches; "
            f"{counts['latest_reentry_zero_match_programs']} new routes remain without a retained match. Across all twelve "
...
def main() -> int:
    ...
    pass2.update({
        ...
        "stage_04_reentry_completed": 12 if result["status"] == "PASS" else 11,
    ...
    update_progress(
        progress_path,
        current_phase="pass2_stage_04_reentry_12_complete" if result["status"] == "PASS" else "pass2_stage_04_reentry_12_failed",
        pass2=pass2,
    )
    ...
    unresolved = [
        ...
        "Stage 5 scoring still covers the prior 120-program roster; all six new routes remain intentionally absent until Stage 5 re-entry 12 is executed.",
    ]
    ...
    manifest = {
        ...
        "schema_version": PASS2_SCHEMA_VERSION,
        "stage": 4,
        "run_type": "faculty_reentry_12",
        "name": "Deep professor and department fit mapping — re-entry 12",
        ...
        "validation": {
            ...
            "full_suite_boundary": {
                ...
                "result": "116 passed, 1 expected downstream Stage 5 coverage failure",
                "unresolved_stages": [5],
            },
        },
        "counts": result["counts"],
