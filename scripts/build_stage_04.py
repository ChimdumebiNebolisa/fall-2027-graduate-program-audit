...
def write_report(result: dict[str, object]) -> None:
    ...
    lines = [
        ...
        "",
        (
            f"- Stage 4 re-entry 03 evaluated {counts['latest_reentry_programs']} newly eligible routes and retained "
            f"{counts['latest_reentry_retained_match_rows']} fully verified strong lead matches; "
            f"{counts['latest_reentry_zero_match_programs']} new routes remain without a retained match. Across all three "
        ...
        "- Stage 5 scoring still covers the prior 63-program roster. Its eight-route coverage gap is intentionally unresolved at this stage boundary and must be rebuilt in Stage 5 re-entry 03.",
...
def main() -> int:
    ...
    pass2.update({
        ...
        "stage_04_reentry_completed": 3 if result["status"] == "PASS" else 2,
    ...
    update_progress(
        progress_path,
        current_phase="pass2_stage_04_reentry_03_complete" if result["status"] == "PASS" else "pass2_stage_04_reentry_03_failed",
        pass2=pass2,
    )
    ...
    unresolved = [
        ...
        "Stage 5 scoring still covers the prior 63-program roster; all eight new routes remain intentionally absent until Stage 5 re-entry 03 is executed.",
    ]
    ...
    manifest = {
        ...
        "schema_version": PASS2_SCHEMA_VERSION,
        "stage": 4,
        "run_type": "faculty_reentry_03",
        "name": "Deep professor and department fit mapping — re-entry 03",
