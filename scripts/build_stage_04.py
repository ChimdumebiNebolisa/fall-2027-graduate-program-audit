...
def write_report(result: dict[str, object]) -> None:
    ...
    lines = [
        ...
        "",
        (
            f"- Stage 4 re-entry 04 evaluated {counts['latest_reentry_programs']} newly eligible routes and retained "
            f"{counts['latest_reentry_retained_match_rows']} fully verified strong lead matches; "
            f"{counts['latest_reentry_zero_match_programs']} new routes remain without a retained match. Across all four "
...
def main() -> int:
    ...
    pass2.update({
        ...
        "stage_04_reentry_completed": 4 if result["status"] == "PASS" else 3,
    ...
    update_progress(
        progress_path,
        current_phase="pass2_stage_04_reentry_04_complete" if result["status"] == "PASS" else "pass2_stage_04_reentry_04_failed",
        pass2=pass2,
    )
    ...
    unresolved = [
        ...
        "Stage 5 scoring still covers the prior 71-program roster; all nine new routes remain intentionally absent until Stage 5 re-entry 04 is executed.",
    ]
    ...
    manifest = {
        ...
        "schema_version": PASS2_SCHEMA_VERSION,
        "stage": 4,
        "run_type": "faculty_reentry_04",
        "name": "Deep professor and department fit mapping — re-entry 04",
        ...
        "validation": {
            ...
            "full_suite_boundary": {
                ...
                "result": "73 passed, 1 expected downstream coverage failure",
                "unresolved_stages": [5],
            },
        },
        "counts": result["counts"],
