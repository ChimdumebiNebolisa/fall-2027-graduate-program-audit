...
def write_report(result: dict[str, object]) -> None:
    ...
    lines = [
        ...
        "",
        "- Stage-specific verification: `python -m pytest tests/test_professor_mapping.py -q` — 24 passed.",
        "- Full-suite boundary: `python -m pytest -q` — 122 passed; the sole failure is the expected Stage 5 coverage gap for five routes.",
        "",
        ...
        "",
        (
            f"- Stage 4 re-entry 13 evaluated {counts['latest_reentry_programs']} newly eligible routes and retained "
            f"{counts['latest_reentry_retained_match_rows']} fully verified strong lead matches; "
            f"{counts['latest_reentry_zero_match_programs']} new routes remain without a retained match. Across all thirteen "
        ...
        "- Stage 5 scoring still covers the prior 126-program roster. Its five-route coverage gap is intentionally unresolved at this stage boundary and must be rebuilt in Stage 5 re-entry 13.",
        ...
        "- `data/raw/pass2/stage_04_reentry_13.json`",
        ...
        "",
        "Proceed to Stage 5 only with the rebuilt Stage 4 depth values. Keep the five new routes out of scored outputs until Stage 5 re-entry 13 regenerates and validates complete scoring coverage.",
        "",
    ]
...
def main() -> int:
    ...
    pass2.update({
        ...
        "stage_04_reentry_completed": 13 if result["status"] == "PASS" else 12,
    ...
    update_progress(
        progress_path,
        current_phase="pass2_stage_04_reentry_13_complete" if result["status"] == "PASS" else "pass2_stage_04_reentry_13_failed",
        pass2=pass2,
    )
    ...
    unresolved = [
        ...
        "Stage 5 scoring still covers the prior 126-program roster; all five new routes remain intentionally absent until Stage 5 re-entry 13 is executed.",
    ]
    ...
    manifest = {
        ...
        "schema_version": PASS2_SCHEMA_VERSION,
        "stage": 4,
        "run_type": "faculty_reentry_13",
        "name": "Deep professor and department fit mapping — re-entry 13",
        ...
        "validation": {
            ...
            "full_suite_boundary": {
                ...
                "result": "122 passed, 1 expected downstream Stage 5 coverage failure",
                "unresolved_stages": [5],
            },
        },
        "counts": result["counts"],
