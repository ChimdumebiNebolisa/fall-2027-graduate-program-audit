...
def validate(
    ...
    assertions = {
        ...
        "latest_reentry_10_receives_dispositions_and_pressure_tests": (
            len(latest_reentry_ids) == len(latest_entries) == 5
...
def write_report(portfolio: dict[str, object], assertions: dict[str, bool], counts: dict[str, int]) -> None:
    ...
    lines = [
        ...
        (
            f"- Stage 6 re-entry 10 pressure-tested all {counts['latest_reentry_programs']} newly scored routes: "
...
def main() -> int:
    ...
    pass2.update({
        ...
        "stage_06_reentry_completed": 10 if status == "PASS" else 9,
    ...
    update_progress(
        progress_path,
        current_phase="pass2_stage_06_reentry_10_complete" if status == "PASS" else "pass2_stage_06_reentry_10_failed",
        pass2=pass2,
    )
    ...
    unresolved = [
        ...
        f"Stage 6 re-entry 10 leaves {counts['latest_reentry_monitor']} of {counts['latest_reentry_programs']} newly scored routes on monitor and {counts['latest_reentry_do_not_apply']} as do-not-apply.",
    ...
    manifest = {
        ...
        "schema_version": "2.0",
        "stage": 6,
        "run_type": "portfolio_reentry_10",
        "name": "Construct and pressure-test the application portfolio — re-entry 10",
