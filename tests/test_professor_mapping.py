...
def test_reentry_17_evidence_exactly_covers_new_faculty_ready_routes():
    raw = json.loads(
        (REPO_ROOT / "data/raw/pass2/stage_04_reentry_17.json").read_text(encoding="utf-8")
    )
    newly_ready = {
        row["candidate_program_id"]
        for row in _rows("stage_03_reentry_17_verification.csv")
        if row["faculty_review_ready"] == "yes"
    }
    professors = raw["professors"]
    assert {row["program_id"] for row in professors} == newly_ready
    assert len(professors) == len(newly_ready) == 9
    assert sum(row["can_supervise_program"].casefold() == "yes" for row in professors) == 8
    unresolved = [row for row in professors if row["can_supervise_program"].casefold() != "yes"]
    assert len(unresolved) == 1
    assert unresolved[0]["institution_name"] == "Åbo Akademi University"
    assert all(row["fit_strength"].casefold() == "strong" for row in professors)
    assert all(row["recent_work_1_url"].startswith("https://") for row in professors)
    assert all(row["recent_work_1_year"].isdigit() for row in professors)
    assert all(row["official_email"] or row["official_faculty_url"] for row in professors)
...
def test_reentry_17_has_typed_source_records_for_every_lead_evidence_url():
    raw = json.loads(
        (REPO_ROOT / "data/raw/pass2/stage_04_reentry_17.json").read_text(encoding="utf-8")
    )
    source_urls = {row["url"].rstrip("/") for row in raw["sources"]}
...
def test_reentry_18_evidence_exactly_covers_new_faculty_ready_routes():
    raw = json.loads(
        (REPO_ROOT / "data/raw/pass2/stage_04_reentry_18.json").read_text(encoding="utf-8")
    )
    newly_ready = {
        row["candidate_program_id"]
        for row in _rows("stage_03_reentry_18_verification.csv")
        if row["faculty_review_ready"] == "yes"
    }
    professors = raw["professors"]
    assert {row["program_id"] for row in professors} == newly_ready
    ...
    assert all(row["recent_work_1_url"].startswith("https://") for row in professors)
    assert all(row["recent_work_1_year"].isdigit() for row in professors)
    assert all(row["official_email"] and row["official_faculty_url"] for row in professors)
    assert all(row["recruiting_status"] in RECRUITING_STATUSES for row in professors)


def test_reentry_18_has_typed_source_records_for_every_lead_evidence_url():
    raw = json.loads(
        (REPO_ROOT / "data/raw/pass2/stage_04_reentry_18.json").read_text(encoding="utf-8")
    )
    source_urls = {row["url"].rstrip("/") for row in raw["sources"]}
