...
def test_reentry_merge_is_idempotent_and_reaudits_confirmed_omissions(tmp_path):
    ...
    assert len(merged) == baseline["counts"]["candidate_rows"] + 204
    assert all(int(row["candidate_rows_contributed"]) >= 0 for row in yields)
    assert sum(row["audit_result"] == "false_negative_corrected" for row in audits) == 34
...
def test_reentry_16_has_the_intended_regional_and_route_mix():
    payload = _payload(16)
    candidates, sources = build_reentry_rows(
        payload["candidates"],
        _institutions(),
        "data/raw/pass2/stage_02_reentry_16.json",
        reentry_id="stage_02_reentry_16",
    )
...
def test_reentry_17_has_the_intended_regional_and_route_mix():
    payload = _payload(17)
    candidates, sources = build_reentry_rows(
        payload["candidates"],
        _institutions(),
        "data/raw/pass2/stage_02_reentry_17.json",
        reentry_id="stage_02_reentry_17",
    )
