...
Generated: 2026-09-11T01:35:01.172499+00:00

Decision: **PASS**

## Outcome

All 83 faculty-review-ready programs were searched against a current official faculty roster or department research-area listing. Exactly five plausible candidates were evaluated for each program (415 program-candidate evaluations; 396 distinct people after university-level deduplication).

The defensible retained set contains 62 program-match rows representing 62 distinct professors. Each program has one fully verified strong match in 62 programs; the other 21 programs have no match that clears every evidence gate. One-match programs receive 5 depth points and zero-match programs receive 0. No individual professor score exists in the Stage 4 outputs.
...
Recent-work metadata was available for 127 of 415 candidate evaluations. Discovery-only OpenAlex records never establish appointment, supervision, or recruiting.
...
- Stage-specific verification: `python -m pytest tests/test_professor_mapping.py -q` — 10 passed.
- Full-suite boundary: `python -m pytest -q` — 77 passed; the sole failure is the expected Stage 5 coverage gap for the three new routes.
...
- Stage 4 re-entry 05 evaluated 3 newly eligible routes and retained 3 fully verified strong lead matches; 0 new routes remain without a retained match. Across all five re-entry batches, 37 of 38 routes have a retained match.
...
- 62 of 83 programs have only one fully verified strong match and remain single-professor dependencies.
...
- 353 plausible program-candidate evaluations were not retained because exact-route supervision authority and/or candidate-specific recent-work evidence remains incomplete.
...
- Stage 5 scoring still covers the prior 80-program roster. Its three-route coverage gap is intentionally unresolved at this stage boundary and must be rebuilt in Stage 5 re-entry 05.
