...
Generated: 2026-09-11T00:38:04.734637+00:00

Decision: **PASS**

## Outcome

All 80 faculty-review-ready programs were searched against a current official faculty roster or department research-area listing. Exactly five plausible candidates were evaluated for each program (400 program-candidate evaluations; 385 distinct people after university-level deduplication).

The defensible retained set contains 59 program-match rows representing 59 distinct professors. Each program has one fully verified strong match in 59 programs; the other 21 programs have no match that clears every evidence gate. One-match programs receive 5 depth points and zero-match programs receive 0. No individual professor score exists in the Stage 4 outputs.
...
Recent-work metadata was available for 120 of 400 candidate evaluations. Discovery-only OpenAlex records never establish appointment, supervision, or recruiting.
...
- Stage-specific verification: `python -m pytest tests/test_professor_mapping.py -q` — 9 passed.
- Full-suite boundary: `python -m pytest -q` — 73 passed; the sole failure is the expected Stage 5 coverage gap for the nine new routes.
...
- Stage 4 re-entry 04 evaluated 9 newly eligible routes and retained 9 fully verified strong lead matches; 0 new routes remain without a retained match. Across all four re-entry batches, 34 of 35 routes have a retained match.
...
- 59 of 80 programs have only one fully verified strong match and remain single-professor dependencies.
...
- 341 plausible program-candidate evaluations were not retained because exact-route supervision authority and/or candidate-specific recent-work evidence remains incomplete.
...
- Stage 5 scoring still covers the prior 71-program roster. Its nine-route coverage gap is intentionally unresolved at this stage boundary and must be rebuilt in Stage 5 re-entry 04.
